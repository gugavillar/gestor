(function () {
	'use strict';
	/*global M, angular, $*/
	function GerarGuiaSaidaCtrl(Departamentos, Produtos, GuiasSaidasResource, EstoqueResource) {
		var vm = this, copy;
		vm.block = false;
		vm.produtosGuia = [];
		...
		function cadastrar() {
			vm.dados.id_usuario_saida = sessionStorage.getItem('id');
			GuiasSaidasResource.guias().save(vm.dados).$promise.then(function (data) {
				if (data.$resolved && data.id_saida) {
					vm.dados.id_saida = data.id_saida;
					M.toast({ html: 'Guia aberta com sucesso', inDuration: 1500, classes: 'rounded noprint', completeCallback: bloqueio(true) });
				} else {
					delete vm.dados;
	...
	GerarGuiaSaidaCtrl.$inject = ['Departamentos', 'Produtos', 'GuiasSaidasResource', 'EstoqueResource'];

	angular.module('GESTOQUE').controller('GerarGuiaSaidaCtrl', GerarGuiaSaidaCtrl);
}());